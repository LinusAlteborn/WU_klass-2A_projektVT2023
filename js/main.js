...
function load_site(name){
    ...
    if (name == "" || name == "index"){
        render_games();
        document.getElementById("load-more").addEventListener('click', function(){load_more_games(8);});
    }
    if (name === "game"){
...
function render_game(element, id) {
    element.href = "game.html?" + id;
    element.getElementsByTagName("h2")[0].innerHTML = games[id].name;
    element.getElementsByTagName("img")[0].src = "img/thumbnails/" + id + ".webp";
    element.getElementsByTagName("p")[0].innerHTML = time_since(games[id].shared);
}

function render_games() {
    for(var i = 0; i < document.getElementById("binder").childElementCount; i++) {
        render_game(document.getElementById("binder").children[i], i);
    }
}
